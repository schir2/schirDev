import FormData from '../interfaces/FormData'
const initialFormData: FormData = {
    age: 38,
    year: 2024,
    lifeExpectancy: 75,
    retirementStrategy: 'age',
    retirementWithdrawalRate: 4,
    retirementIncomeGoal: 100000,
    retirementAge: 40,
    retirementSavingsAmount: 2000000,
    retirementIncomeProjected: 0,
    cash: 27000,
    incomePreTaxed: 200000,
    incomeTaxable: 0,
    incomeTaxed: 0,
    incomeDisposable: 0,
    incomeGrowthStrategy: 'percentage_increase',
    incomeGrowthRate: 0,
    incomeGrowthAmount: 0,
    incomeTaxAmount: 0,
    incomeTaxRate: 30.0,
    incomeTaxStrategy: 'simple',
    incomeTaxFilingStatus: 'single',
    incomeTaxNumberOfDependents: '0',
    investmentGrowthStrategy: 'start',
    expenses: 50000,
    expenseRate: 2,
    expensesGrowthStrategy: 'fixed',
    debt: 5000,
    debtInterestRate: 5,
    debtPayment: 500,
    debtPaymentTotal: 0,
    targetAnnualRetirementSpendingPreTaxed: 0,
    targetEndOfLifeSavingsPreTaxed: 0,
    taxDeferredSavingsStartOfYear: 0,
    taxDeferredContributionFixedAmount: 5000,
    taxDeferredContributionStrategy: 'max',
    taxDeferredSavingsEndOfYear: 0,
    taxDeferredContribution: 0,
    taxDeferredContributionTotal: 0,
    taxDeferredContributionPercentage: 6,
    taxDeferredGrowthRate: 6.0,
    taxDeferredContributionElectiveLimit: undefined,
    taxDeferredContributionElectiveLimitApplied: undefined,
    taxDeferredContributionCatchUpLimit: undefined,
    taxDeferredContributionTotalElectiveLimit: undefined,
    taxDeferredContributionElectiveTotalLimitApplied: undefined,
    taxDeferredContributionElectiveLimitInflationRate: 2.5,
    employerContributionStrategy: 'percentage_of_contribution',
    employerContribution: 0,
    employerMatchPercentage: 100,
    employerMatchPercentageLimit: 3,
    employerContributionPercentage: 0,
    employerContributionFixedAmount: 0,
    employerSavingsEndOfYear: 0,
    employerSavingsStartOfYear: 0,

    iraTaxableSavingsStartOfYear: 0,
    iraTaxableSavingsEndOfYear: 0,
    iraTaxableContribution: 0,
    iraTaxableContributionFixedAmount: 0,
    iraTaxableContributionStrategy: 'max',
    iraTaxableContributionPercentage: 0,

    iraTaxDeferredSavingsStartOfYear: 0,
    iraTaxDeferredSavingsEndOfYear: 0,
    iraTaxDeferredContribution: 0,
    iraTaxDeferredContributionFixedAmount: 0,
    iraTaxDeferredContributionStrategy: 'max',
    iraTaxDeferredContributionPercentage: 0,

    iraGrowthRate: 6.0,
    iraContributionLimit: 7000,
    iraContributionCatchUpLimit: 8000,

    taxableContributionFixedAmount: 10,
    taxableContribution: 0,
    taxableContributionStrategy: 'fixed',
    taxableSavingsStartOfYear: 0,
    taxableSavingsEndOfYear: 0,
    taxableContributionLifetime: 0,
    taxableContributionPercentage: 0,
    taxableGrowthRate: 10.0,
    inflationRate: 2.0,
    inflationGrowthStrategy: 'fixed',
    allowNegativeBalance: false,
};

export default initialFormData;
